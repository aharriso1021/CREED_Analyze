function [data_non_filtered_gaze_and_blink_corrected, blink_parameters] = CREED_blink_correction3(data_non_filtered, trial_i, prm)
...
% 
saccade_latency = prm.blink.sacc_init;
foveal_radius_vision = prm.blink.fov_rad;
saccade_durations = prm.blink.sacc_dur;
blink_latency = prm.blink.blink_init;
max_blink_duration = prm.blink.blink_dur;
pre_blink = prm.blink.pre_blink;
post_blink = prm.blink.post_blink;
%
...
%% Find small nonnan segments between blinks (set to 10 but can be modified based on need)
i_short = find_short_nonnan_segments2(blinks,50);
blinks(i_short) = true;

%% Pad -25/+75ms to each blink - tweaked to handle multiple blinks 
blink_diff = [0 ; diff(blinks)];
blinki = find(blink_diff == 1) - pre_blink; % indeex of all FIRST NaN in Blink  & pad
blinko  = (find(blink_diff == -1)-1) + post_blink; % index of all LAST NaN in Blink & pad
    if length(blinko) < length(blinki) % blink duration exceeds end of data
        blinko = [blinko ; length(blinks)];
    elseif length(blinki) < length(blinko) % blink at beginning 
        blinki = [1 ; blinki];   
    end

blinki(blinki < 0) = 1;
blinko(blinko > length(blinks)) = length(blinks);
 
for k = 1:length(blinki)
    blinks(blinki(k) : blinko(k)) = true;
end
...
blink_parameters.durations = (blink_durations - pre_blink - post_blink) * (1e3/robot_Fs);
...
[X] = fill_saccade_fix2(X, 'gaze', foveal_radius_vision, blink_init, blink_offs,...
   saccade_onsets, saccade_ends, trial_i); % Fills the gaps due to blinks with a saccade and fixation.

[Y] = fill_saccade_fix2(Y, 'gaze', foveal_radius_vision, blink_init, blink_offs,...
   saccade_onsets, saccade_ends, trial_i);

[P] = fill_saccade_fix2(P, 'pupil',foveal_radius_vision, blink_init, blink_offs, [], [], trial_i); % See above amd CREED_gaze_config_parameters for max_blink_duration and blink_latency
