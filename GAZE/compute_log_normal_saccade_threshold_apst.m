function varargout= compute_log_normal_saccade_threshold_apst(varargin)
...
Velocity_Threshold_Vector=[exp(0.5*((1*lower_saccade_vel)+(1*upper_fixation_vel))),20];
Velocity_Threshold=max(Velocity_Threshold_Vector);%If the distribution is not bimodal, the threshold could be affected. 
% Velocity_Threshold=max(10^lower_saccade_vel,30); %If the distribution is not bimodal, the threshold could be affected. 

% if Velocity_Threshold>35
%     Velocity_Threshold=35;
% end

varargout{4}=Velocity_Threshold;
...
varargout{5}=max(Velocity_Threshold_Vector);
%% 
% figure('Color',[1 1 1]);
% PeakSig=Data_Vector(2001:8000);
